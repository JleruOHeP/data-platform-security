...
The assessment interview is driven by the generated `controls.json` file.
...
1. Load every control from `controls.json`.
2. Discover its canonical name and aliases.
...
1. Read `controls.json`.
2. Discover every entry in its `controls` array.
3. Build the interview from those controls.

Never hardcode control names inside this skill.

`controls.json`, generated by `seed_neo4j.py`, is the interview source of truth.

If the seed model changes, run `seed_neo4j.py` again so the interview inventory is regenerated.
...
- controls.json cannot be loaded
...
Do not modify `controls.json` during an interview.

`controls.json` is the authoritative interview inventory; Neo4j is the runtime model source of truth.

The interview should automatically adapt whenever `controls.json` is regenerated.
